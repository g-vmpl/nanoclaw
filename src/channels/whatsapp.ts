...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    // Saved reference to the cacheable signal key store so groups.update
    // can clear sender-key-memory and force SKDM redistribution.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let signalKeyStore: { set(data: Record<string, any>): void | Promise<void> } | undefined;
    ...
    async function getNormalizedGroupMetadata(jid: string): Promise<GroupMetadata | undefined> {
      ...
      // Return raw metadata with LID JIDs intact — Baileys v7 resolves LID
      // internally via signalRepository.lidMapping when encrypting sender keys.
      // Pre-translating here breaks that lookup and causes SKDM desync.
    ...
    async function connectSocket(): Promise<void> {
      ...
      // WA fires groups.update (often without a subject change) after each group send
      // to signal that sender key state has changed. Clear metadata cache and sender
      // key memory so Baileys redistributes SKDMs to all participants on next send.
      sock.ev.on('groups.update', (updates) => {
        for (const update of updates) {
          if (!update.id) continue;
          groupMetadataCache.delete(update.id);
          if (signalKeyStore) {
            void Promise.resolve(signalKeyStore.set({ 'sender-key-memory': { [update.id]: null } })).catch((err) =>
              log.debug('Failed to clear sender key memory', { jid: update.id, err }),
            );
